...
type LowFloorBusRouteCardProps = {
  route: LowFloorBusRouteResult
}
...
function LowFloorBusRouteCard({ route }: LowFloorBusRouteCardProps) {
  if (route.status === 'unavailable') {
    return (
      <section className="route-result-card bus-result-card unavailable" aria-labelledby="bus-result-title">
        <div className="result-heading">
          <span className="transport-badge bus" aria-hidden="true">저상버스</span>
          <div>
            <p className="result-kicker">경로를 찾지 못했어요</p>
            <h2 id="bus-result-title">저상버스 경로</h2>
          </div>
        </div>
        <p className="result-message">{route.unavailable_reason}</p>
      </section>
    )
  }
  ...
  return (
    <section className="route-result-card bus-result-card" aria-labelledby="bus-result-title">
      <div className="result-heading">
        <span className="transport-badge bus" aria-hidden="true">저상버스</span>
        <div>
          <p className="result-kicker">노선 단위 저상버스 정보가 포함된 경로</p>
          <h2 id="bus-result-title">{route.summary ?? '저상버스 경로'}</h2>
        </div>
      </div>
      ...
      <div className="accessibility-note">
        ...
        {route.warnings.length > 0 ? (
          ...
          <p>현재 경로에서 별도의 저상버스·접근성 주의사항이 확인되지 않았습니다.</p>
        )}
      </div>
    </section>
  )
...
export default LowFloorBusRouteCard
