...
- 한 일:
  ...
  - 서울 밖 장소 선택 시 Backend 추천 API를 호출하지 않는 렌더 테스트를 추가하고, 기존 bounding box라면 통과할 수 있던 경기도 주소 케이스도 차단하도록 검증했다.
...
- 자체 리뷰:
  ...
  - 서울 서비스 범위는 Kakao Places 결과의 주소가 `서울` 또는 `서울특별시`로 시작하는지 확인하는 UI guard이며, 별도 행정경계 polygon이나 실제 서비스 권역 판정 API를 새로 만들지는 않았다.
