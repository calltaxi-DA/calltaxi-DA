...
- 필요한 일:
  - 서울 버스위치정보조회 API 실제 호출 성공 여부 확인
  - 차량 ID, 차량유형, 혼잡도, 저상버스 여부 관련 필드 실제 값과 코드 의미 확인
  - 서울 버스도착정보조회 API 실제 응답 확보
  - 정류장 ID, ARS ID, ODsay 정류장 ID mapping 가능성 확인
  - 배차간격이 공식 필드로 제공되는지 확인하고, 없으면 실시간 위치·도착정보 기반 headway 계산 가능성 검증
  - 교통카드 빅데이터의 공식 재차인원/혼잡도 데이터 권한, 갱신주기, 호출 제한 확인
  - 공식 확보가 불가능한 항목을 최종 판정하고, 필요한 경우에만 크롤링 검토 범위 분리
