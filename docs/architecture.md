...
> 요금/시간/도보 우선순위에 따른 **추천 로직(Rule-based ranking)은 `ai/`가 아니라 `backend/`가 담당한다.** Backend Phase 7에서 경로 지표 가용 상태와 접근성 상태를 포함한 추천 계약 및 정렬 서비스를 구현했다(ADR 0004).
...
> 화면 구성(MAP/경로비교/캘린더)의 전체 스펙은 아직 확정되지 않았다. 이동수단 경로 비교의 공통 응답 계약(`Location`, `TransportType`, `RouteStatus`, `RouteResult`, `RouteComparisonResponse`)과 추천 요청·응답 계약은 확정했다. `RouteResult`는 경로 상태와 별도로 numeric field별 가용 상태 및 접근성 확인 상태를 표현한다(ADR 0004).
