...
`calltaxi-DA`는 하나의 모노레포 안에 서로 다른 두 종류의 산출물을 담는다.
...
두 축은 서로 다른 실행 주기(1회성 분석 vs 상시 서비스)와 데이터 신선도 요구사항을 가지므로 분리되어 있다. 분석 결과(예: 시간대별 대기시간 통계)가 서비스에 필요해지면, `ai/`가 그 결과를 lookup 테이블/모델 형태로 가공해 가져오는 구조를 취한다 — 서비스가 노트북이나 원본 CSV를 직접 읽지 않는다.
...
- **`ai/`** — 장애인콜택시 대기시간 산출과 Rule-based 추천 로직. 순수 Python이며 FastAPI/HTTP를 알지 못한다(단독 테스트·재사용 가능해야 함).
- **`frontend/`** — 사용자 화면. `backend/`가 노출하는 API만 호출하고, 데이터 파일이나 `ai/`를 직접 참조하지 않는다.
- **`data/`, `notebooks*/`, `src/`** — 기존 데이터분석 자산. 서비스 코드가 이 폴더의 파일 경로를 하드코딩해 참조하지 않는다.
- **`docs/`** — 이 저장소를 다루는 데 필요한 설계/운영 문서. 노트북 단위의 분석 계획 문서는 `notebooks_docs_lye/`(gitignore 대상, 팀 공유 문서 아님)에 남는다.

## 데이터 소유권

- 장애인콜택시 원본/전처리 데이터(`data/raw`, `data/processed`)의 소유권과 갱신 책임은 분석 축에 있다. 서비스가 이 데이터의 최신 값이 필요하면 `ai/`에 명시적으로 값을 복사/재계산해 넣고, 원본 CSV 경로를 서비스 코드에서 직접 열지 않는다.
