...
> MAP/경로비교 화면 계약과 교통비 기록·월별 비교 계약을 구현했다. `RouteResult`는 경로 상태와 별도로 numeric field별 가용 상태 및 접근성 확인 상태를 표현한다. 공개 추천 요청은 출발지·목적지·선택 이동수단·우선순위만 받고 경로 결과는 Backend provider가 생성한다. 교통비 기록 시에도 Backend가 동일 경로 조건을 금액 우선으로 다시 계산한다(ADR 0004, ADR 0005).
...
- 서비스가 생성하는 교통비 기록은 `backend/`가 소유하며 로컬 SQLite에 저장한다. 사용자가 입력한 실제 이용금액과 Backend가 저장 시점에 다시 산출한 금액 우선 추천 비용을 함께 보존한다. `frontend/`는 이 값을 계산하거나 별도 저장하지 않는다. 현재는 인증 없는 단일 서비스 인스턴스 범위이며 DB 파일은 분석 export가 아니다(ADR 0005).
